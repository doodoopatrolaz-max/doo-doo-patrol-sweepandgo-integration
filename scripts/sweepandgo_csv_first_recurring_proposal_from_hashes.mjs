...
function main() {
  ...
  if (!csvPath || !hashPath || !outputPath) {
    throw new Error(
      "Usage: sweepandgo_csv_first_recurring_proposal_from_hashes.mjs <csv-path> <hash-export-path> <proposal-output-path>"
    );
  }
  ...
  const customersWithExistingDates = new Set(customerRows.filter((row) => row.firstRecurringDate).map((row) => row.customerKey));
  ...
  for (const record of records) {
    ...
    if (emailMatches.length === 1) {
      ...
      summary.existingBiCustomersMatchedByEmail += 1;
    } else if (emailMatches.length > 1) {
      summary.ambiguousMatches += 1;
      continue;
    } else if (phoneMatches.length === 1) {
      match = phoneMatches[0];
      summary.existingBiCustomersMatchedByPhone += 1;
    } else if (phoneMatches.length > 1) {
      summary.ambiguousMatches += 1;
      continue;
    } else {
    ...
    if (!customersWithExistingDates.has(match.customerKey) && !proposedCustomerDates.has(match.customerKey)) {
      proposedCustomerDates.set(match.customerKey, createdAt);
    }
  }

  summary.customersEligibleForFirstRecurringDateBackfill = proposedCustomerDates.size;
  summary.monthToDateNewRecurringCustomersAfterProposedBackfill = [
    ...customerRows.filter((row) => row.firstRecurringDate).map((row) => row.firstRecurringDate),
    ...proposedCustomerDates.values()
  ].filter((date) => inRange(date, range)).length;
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      version: 1,
      summary,
      updates: [...proposedCustomerDates.entries()].map(([customerKey, firstRecurringDate]) => ({
        customerKey,
        firstRecurringDate
      }))
    })}\n`
